...
| Sell a model from a product page | `products` → set `website_slug` to the page (`solo-flex`, `duo`, `modular-xl`, `interior`, `divide`, `solid`). Solo ECO, Solo Stand and Modular 4 are in the catalogue but have no page yet, so they carry no slug. |
