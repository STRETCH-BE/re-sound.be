...
export default async function Page({ params: { locale } }: PageProps) {
  ...
  const crumbs = await productCrumbs(locale, 'rwood-veneer', cleanName);
  ...
  // Product FAQ (messages) + the workbook rows tagged "rwood-veneer", without duplicates
  const faqEntries: FaqEntry[] = mergeFaqEntries(
    ...
    (await faqForResolved(locale, 'rwood-veneer')).map((f) => ({ question: f.question, answer: f.answer }))
  );
  ...
  return (
    <NextIntlClientProvider locale={locale} messages={messages}>
      ...
      <RWoodVeneerProductPage
        ...
        otherModels={<OtherModels slug="rwood-veneer" locale={locale} />}
      />
