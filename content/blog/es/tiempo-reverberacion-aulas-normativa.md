---
id: "BP-classroom-reverberation-es"
slug: "tiempo-reverberacion-aulas-normativa"
locale: "es"
title: "Reverberación en aulas: normas, medición y solución | Re-Sound"
description: "Qué tiempo de reverberación necesita un aula, cómo se mide y cuánta absorción hace falta: ejemplo calculado y soluciones que aguantan en un colegio."
h1: "Tiempo de reverberación en las aulas: normas, mediciones y soluciones"
keyword: "tiempo de reverberación aula"
secondaryKeywords: ["acústica en aulas","paneles acústicos para colegios","reverberación en colegios","cte db-hr aulas"]
intent: "learn"
category: "education"
author: {"name":"Michael Nicasens","jobTitle":"CEO Stretch Group"}
datePublished: "2026-09-20"
dateModified: "2026-09-20"
heroImage: "/images/products/rpet-groove/hero-rpet-groove.webp"
heroAlt: "Aula con paneles rPET Groove en la pared del fondo y un techo absorbente"
internalLinks: ["/es/acoustic-calculator","https://stretchplafond.be/products/acoustic-stretch-system","/es/products/rpet-groove","/es/products/interior","/es/products/rwood-micro","/es/products/divide","/es/products/pet-acoustic-panels","/es/samples","/es/where-to-buy"]
cta: "Solicite el texto de prescripción y un kit de muestras para su centro."
sources: "NBN S 01-400-2:2012 (criterios acústicos para edificios escolares, Bélgica); DIN 18041:2016 (acústica de recintos, Alemania); CTE DB-HR como referencia en España; ISO 3382-2 (medida del tiempo de reverberación); ISO 11654 (clases A a E de absorción); EN 13501-1 (clasificación al fuego); fichas de producto de Re-Sound."
faq: [{"question":"¿Qué tiempo de reverberación es obligatorio en un aula?","answer":"La norma belga NBN S 01-400-2 impone un tiempo de reverberación nominal máximo por tipo de recinto y volumen; para un aula estándar eso equivale en la práctica a unos 0,6 a 0,8 s (confort normal), más estricto para el confort elevado. En España la referencia es el CTE DB-HR; haga calcular la exigencia exacta a su arquitecto o acústico."},{"question":"¿Bastan los paneles de pared en un aula?","answer":"Normalmente no, salvo ocupando mucha superficie de pared: el techo es la mayor superficie y la más eficiente. Techo más pared del fondo es la combinación que cumple la norma en la mayoría de las aulas."},{"question":"¿Con qué rapidez se puede instalar en un colegio?","answer":"El rPET tarda aproximadamente 3 o 4 semanas laborables; un aula media la instala un equipo en media jornada. Haga el pedido antes de finales de mayo para instalar con seguridad en las vacaciones de verano."}]
draft: false
translationKey: "classroom-reverberation"
model: "nagalmtijd-klaslokaal-normen"
wordCount: 1507
---

**En un aula, el tiempo de reverberación decide si los alumnos entienden al profesor y si el profesor conserva la voz a las cuatro de la tarde.** La norma belga NBN S 01-400-2 fija tiempos de reverberación máximos para los edificios escolares; en la práctica, para un aula corriente eso se traduce en unos 0,6 a 0,8 segundos, un orden de magnitud habitual también fuera de Bélgica, mientras que un aula sin tratar (yeso, cristal, baldosas) suele superar 1 segundo. Aquí: qué pide la norma, cómo se mide, cuánta absorción hace falta y qué soluciones aguantan diez cursos.

## Por qué la reverberación en un aula cuesta más de lo que parece

La inteligibilidad cae con rapidez en cuanto la reverberación supera 0,8 s: las sílabas del profesor se solapan, sobre todo al fondo del aula y para los alumnos con otra lengua materna o con peor audición. El profesor compensa hablando más alto (el efecto Lombard), los alumnos también, y el aula suena todavía más fuerte. El resultado se conoce por los estudios sobre problemas de voz del profesorado y por las quejas de concentración en los espacios abiertos. La solución cuesta menos que las quejas: añadir absorción en el techo y en la pared del fondo.

## Qué pide la norma: la referencia belga y el caso español

NBN S 01-400-2 (2012) recoge los criterios acústicos de los edificios escolares belgas: aislamiento a ruido aéreo entre recintos, ruido de impacto, ruido de instalaciones y **tiempo de reverberación** por tipo de recinto. Para la reverberación:

- La exigencia es un **tiempo de reverberación nominal máximo** que depende del tipo de recinto y de su volumen; se evalúa en el recinto amueblado y sin ocupar.
- La norma distingue dos niveles: **confort acústico normal** (la exigencia básica) y **confort acústico elevado** (más estricto, indicado para alumnos con problemas de audición o de lenguaje).
- Además de las aulas, hay exigencias propias para comedores, gimnasios, pasillos y salones de actos, los recintos donde la reverberación suele ser más alta.

Para un aula estándar de 180 a 250 m³, la exigencia belga queda en la práctica entre 0,6 y 0,8 s. Como comparación, la norma alemana DIN 18041 calcula para las aulas T = 0,32·log(V) − 0,17, es decir, 0,57 s para 200 m³: el mismo orden de magnitud. En España la referencia es el CTE DB-HR. Haga calcular la exigencia exacta de su aula al arquitecto o al acústico: volumen y uso determinan la cifra. Ninguna norma dice qué material usar; eso lo decide el centro.

## Cómo se mide el tiempo de reverberación (y qué se pierde si no se mide)

Una medición según ISO 3382-2 ocupa media hora por aula: un impulso breve o ruido, un micrófono en varias posiciones y una lectura por banda de octava de 125 a 4 000 Hz. Lo que se aprende así y no con una estimación:

- **Por frecuencia**: muchas aulas reverberan sobre todo en graves, donde los paneles finos hacen poco; entonces tocan paneles de más espesor o una cámara de aire.
- **Vacía frente a amueblada**: la norma evalúa el recinto amueblado. Armarios, mesas y cortinas cuentan, pero menos de lo que se espera.
- **Antes y después**: medir antes y después de las obras es la prueba para la dirección del centro y, si hay subvención, para la administración.

Medimos previa solicitud; para una primera aproximación nos bastan las medidas del aula.

## Ejemplo de cálculo: de 1,2 a 0,6 segundos

Aula de 8 por 7 m, 3,2 m de altura libre, 179 m³; suelo de baldosas, techo de yeso, una pared de cristal. Medición: **1,2 s**; objetivo: **0,6 s**.

Con la fórmula de Sabine:

- Absorción existente: **24 m² Sabine** (0,161 × 179 / 1,2)
- Absorción para 0,6 s: **48 m² Sabine** (0,161 × 179 / 0,6)
- Hay que añadir **24 m² Sabine**

Propuesta: un **techo absorbente de 30 m²** de clase A (αw 0,9), que aporta 27 m² Sabine, más **6 m² de rWood Groove** en la pared del fondo (αw 0,90, 5,4 m² Sabine). En total, más de 32 m² Sabine, bastante por encima de los 24 necesarios: el aula queda por debajo de 0,6 s incluso vacía y hay margen para los graves. Rehaga el cálculo para su aula con la [calculadora acústica](/es/acoustic-calculator).

## Soluciones que aguantan en un colegio

Un colegio no es una oficina: se apoyan en la pared, arrastran sillas y limpian con esponjas mojadas. Nuestras recomendaciones por zona:

- **El techo, primero.** Un techo tensado acústico ([véalo en STRETCH](https://stretchplafond.be/products/acoustic-stretch-system), nuestra empresa hermana, que lo instala en un día y sin juntas) o paneles de techo de PET reciclado eliminan la mayor parte de la reverberación sin ocupar pared.
- **Pared del fondo y pared lateral.** [rPET Groove](/es/products/rpet-groove): diez colores de serie y colores a medida por encargo, 12 / 24 / 36 mm (NRC de 0,55 a 0,90), clasificación al fuego B-s1,d0 para blanco, gris y negro, B-s2,d0 para los demás colores, se mecaniza como el MDF y se cambia panel a panel. A la altura del oído de los alumnos, no sobre los armarios.
- **Aulas de infantil y de apoyo.** [Interior](/es/products/interior): paneles textiles modulares (αw 1,0, clase A) con funda extraíble y lavable.
- **Comedor, salón de actos y sala polivalente.** [rWood Micro](/es/products/rwood-micro): chapa microperforada de madera, B-s1,d0, con la presencia que pide un salón de actos y la absorción que pide un comedor.
- **Espacios de aprendizaje abiertos.** [Divide](/es/products/divide): mamparas autoportantes acoplables magnéticamente que delimitan zonas de aprendizaje sin levantar tabiques.

## Reacción al fuego, emisiones y mantenimiento

En un edificio escolar, la clase de fuego y la calidad del aire interior importan tanto como la acústica:

- **Reacción al fuego (EN 13501-1)**: toda la gama rPET (Panel, Groove, Flex Groove) es B-s1,d0 si el panel es blanco, gris o negro y B-s2,d0 en cualquier otro color, es decir, contribución limitada al fuego, poco humo y sin gotas inflamadas; los tres rWood (Groove, Micro, Perf) son B-s1,d0 con su núcleo de MDF ignífugo.
- **Emisiones**: certificación OEKO-TEX® Standard 100 para el fieltro rPET.
- **Mantenimiento**: el [fieltro de PET](/es/products/pet-acoustic-panels) se aspira y las manchas se tratan localmente; la funda de Interior se lava en la lavadora; un panel dañado se cambia sin repintar la pared.
- **Origen**: la chapa de todos los productos rWood tiene certificación FSC®, certificado de cadena de custodia C191539.

## Planificación: vacaciones de verano, pliego y recogida

La mayoría de los centros instala la acústica en las vacaciones de verano. Cuente hacia atrás desde el primer día de clase:

1. **Medición y propuesta** en primavera; aprobación por la dirección del centro.
2. **Pliego**: de cada producto entregamos un texto de prescripción con αw, clase, reacción al fuego, formato, espesor y material, para licitar sin nombre de marca.
3. **Pedido antes de finales de mayo**: el rPET tarda aproximadamente 3 o 4 semanas laborables (colores a medida, previa consulta); rWood Micro y Perf, 3 o 4 semanas con chapa de stock y 6 u 8 semanas con chapa fuera de stock; la fecha de rWood Groove se confirma con el pedido.
4. **Instalación** en julio, medición en agosto, listo para septiembre.

Al final de su vida útil, o con motivo de una reforma, recogemos los paneles sin coste en Bélgica, Alemania, Francia, Luxemburgo y los Países Bajos, y el material vuelve a nuestra producción; un colegio en España no puede acogerse a esa recogida gratuita. Re-Sound, marca del grupo Stretch Group, fabrica en sus centros de Beveren-Waas (Bélgica) y de Częstochowa (Polonia).

**¿Empezamos?** Solicite el [kit de muestras gratuito](/es/samples) (muestras A4 de cada gama, enviadas en un plazo de tres días laborables) y los textos de prescripción para su expediente, o, si viaja a Bélgica, visite nuestra [sala de exposición](/es/where-to-buy) en Beveren-Waas.

## Preguntas frecuentes

**¿Qué tiempo de reverberación es obligatorio en un aula?**
La norma belga NBN S 01-400-2 impone un tiempo de reverberación nominal máximo por tipo de recinto y volumen; para un aula estándar eso equivale en la práctica a unos 0,6 a 0,8 s (confort normal), más estricto para el confort elevado. En España la referencia es el CTE DB-HR; haga calcular la exigencia exacta a su arquitecto o acústico.

**¿Bastan los paneles de pared en un aula?**
Normalmente no, salvo ocupando mucha superficie de pared: el techo es la mayor superficie y la más eficiente. Techo más pared del fondo es la combinación que cumple la norma en la mayoría de las aulas.

**¿Con qué rapidez se puede instalar en un colegio?**
El rPET tarda aproximadamente 3 o 4 semanas laborables; un aula media la instala un equipo en media jornada. Haga el pedido antes de finales de mayo para instalar con seguridad en las vacaciones de verano.

*Fuentes: NBN S 01-400-2:2012 (criterios acústicos para edificios escolares, Bélgica); DIN 18041:2016 (acústica de recintos, Alemania); CTE DB-HR como referencia en España; ISO 3382-2 (medida del tiempo de reverberación); ISO 11654 (clases A a E de absorción); EN 13501-1 (clasificación al fuego); fichas de producto de Re-Sound.*
