---
id: "BP-005"
slug: "nagalmtijd-klaslokaal-normen"
locale: "nl"
title: "Nagalmtijd in klaslokalen: norm, meting en oplossing | Re-Sound"
description: "Wat zegt NBN S 01-400-2 over nagalm in klaslokalen, hoe meet je en hoeveel absorptie is nodig? Rekenvoorbeeld en oplossingen die in een school standhouden."
h1: "Nagalmtijd in klaslokalen: normen, metingen en oplossingen"
keyword: "nagalmtijd klaslokaal norm"
secondaryKeywords: ["akoestiek klaslokaal","NBN S 01-400-2 nagalmtijd","akoestische panelen school","nagalm school verbeteren"]
intent: "learn"
category: "education"
author: {"name":"Michael Nicasens","jobTitle":"CEO Stretch Group"}
plannedDate: "2026-12-01"
datePublished: "2026-09-06"
dateModified: "2026-09-20"
heroImage: "/images/products/rpet-groove/hero-rpet-groove.webp"
heroImageMissing: "/images/blog/nagalmtijd-klaslokaal.jpg"
heroAlt: "Klaslokaal met rPET Groove panelen op de achterwand en een absorberend plafond"
internalLinks: ["/nl/products/pet-akoestische-panelen", "/nl/products/rpet-groove", "/nl/products/interior", "/nl/products/rwood-micro", "/nl/products/divide", "https://stretchplafond.be/products/acoustic-stretch-system", "/nl/where-to-buy", "/nl/blog/hoeveel-akoestische-panelen-nodig"]
cta: "Vraag de bestektekst en een stalenpakket voor je school"
sources: "NBN S 01-400-2:2012 (akoestische criteria voor schoolgebouwen); DIN 18041:2016 (Hörsamkeit in Räumen); ISO 3382-2 (meting van de nagalmtijd); ISO 11654 (absorptieklassen); EN 13501-1 (brandclassificatie); productgegevens Re-Sound."
faq: [{"question":"Welke nagalmtijd is verplicht in een klaslokaal?","answer":"NBN S 01-400-2 legt per lokaaltype en volume een maximale nominale nagalmtijd op; voor een standaardklas komt dat in de praktijk neer op ongeveer 0,6 tot 0,8 s (normaal comfort), strenger voor verhoogd comfort. Laat de exacte eis berekenen door je architect of akoesticus."},{"question":"Volstaan wandpanelen alleen in een klas?","answer":"Meestal niet zonder veel wandoppervlak in te nemen: het plafond is het grootste vlak en het efficiëntst. Plafond plus achterwand is de combinatie die in de meeste klassen de norm haalt."},{"question":"Hoe snel kan een school geplaatst worden?","answer":"rPET leveren we in ongeveer 3 à 4 werkweken; een gemiddelde klas plaatst een team op een halve dag. Bestel vóór eind mei om zeker in de zomervakantie te plaatsen."}]
draft: false
translationKey: "classroom-reverberation"
model: "original"
wordCount: 1286
---

**In een klaslokaal beslist de nagalmtijd of leerlingen de leerkracht verstaan en of de leerkracht om 16 uur nog stem heeft.** De Belgische norm NBN S 01-400-2 legt voor schoolgebouwen maximale nagalmtijden vast; in de praktijk komt dat voor een gewoon klaslokaal neer op ongeveer 0,6 tot 0,8 seconden, terwijl een onbehandeld lokaal met gips, glas en een tegelvloer vaak boven 1 seconde zit. Dit artikel legt uit wat de norm vraagt, hoe je meet, hoeveel absorptie je nodig hebt en welke oplossingen tien schooljaren meegaan.

## Waarom nagalm in een klas meer kost dan je denkt

Spraakverstaanbaarheid daalt snel zodra de nagalm boven 0,8 s komt: de lettergrepen van de leerkracht lopen in elkaar over, zeker achteraan in de klas en voor leerlingen die een andere thuistaal spreken of minder goed horen. De leerkracht compenseert door luider te spreken (het Lombard-effect), de leerlingen ook, en het lokaal wordt nog luider. Het resultaat is bekend uit onderzoek naar stemklachten bij leerkrachten en uit klachten over concentratie in open leeromgevingen. De oplossing is goedkoper dan de klachten: absorptie toevoegen op het plafond en op de achterwand.

## De norm in België: NBN S 01-400-2

NBN S 01-400-2 (2012) bevat de akoestische criteria voor schoolgebouwen. De norm behandelt de luchtgeluidisolatie tussen lokalen, het contactgeluid, het lawaai van installaties en de **nagalmtijd** per lokaaltype. Voor de nagalmtijd geldt:

- De eis is een **maximale nominale nagalmtijd** die afhangt van het lokaaltype en het volume; ze wordt beoordeeld in het ingerichte, onbezette lokaal.
- De norm kent twee niveaus: **normaal akoestisch comfort** (de basiseis) en **verhoogd akoestisch comfort** (strenger, aangewezen voor leerlingen met gehoor- of taalproblemen).
- Naast klaslokalen zijn er aparte eisen voor onder meer refters, sportzalen, gangen en aula's — ruimtes waar de nagalm traditioneel het hoogst is.

Voor een standaardklas van 180 tot 250 m³ landt de eis in de praktijk tussen 0,6 en 0,8 s. Ter vergelijking: de Duitse norm DIN 18041 rekent voor lesruimtes met T = 0,32·log(V) − 0,17, wat voor 200 m³ op 0,57 s uitkomt. Laat de exacte eis voor jouw lokaal berekenen door de architect of akoesticus; het volume en de bestemming bepalen het cijfer. Wat de norm niet doet, is zeggen welk materiaal je moet gebruiken — dat is de keuze van de school.

## Hoe je nagalmtijd meet (en wat je mist als je niet meet)

Een nagalmtijdmeting volgens ISO 3382-2 duurt per lokaal een halfuur: een korte impuls of ruis, een microfoon op enkele posities, en een meting per octaafband van 125 tot 4000 Hz. Wat je daaruit leert en niet uit een schatting:

- **Per frequentie**: veel lokalen galmen vooral in de lage tonen, waar dunne panelen weinig doen. Dan kies je dikkere panelen of een luchtspouw.
- **Leeg versus ingericht**: de norm beoordeelt het ingerichte lokaal. Kasten, tafels en gordijnen tellen mee, maar minder dan men hoopt.
- **Voor en na**: een meting vóór de werken en één erna is het bewijs voor het schoolbestuur en, bij subsidiedossiers, voor de subsidiërende overheid.

Bij Re-Sound meten we op aanvraag; voor een eerste raming volstaan de afmetingen van het lokaal.

## Rekenvoorbeeld: van 1,2 naar 0,6 seconden

Klas van 8 × 7 m, 3,2 m hoog (179 m³), tegelvloer, gipsplafond, één glaswand. Gemeten: **1,2 s**. Doel: **0,6 s**.

Met de formule van Sabine (A = 0,161 × V / T):

- A nu = 0,161 × 179 / 1,2 = **24 m² sabine**
- A doel = 0,161 × 179 / 0,6 = **48 m² sabine**
- Toe te voegen: **24 m² sabine**

Voorstel: **30 m² absorberend plafond** van klasse A (αw 0,9), goed voor 27 m² sabine, plus **6 m² rWood Groove** op de achterwand (αw 0,90, 5,4 m² sabine). Samen ruim 32 m² sabine: ruim boven de 24 die nodig zijn, zodat het lokaal ook leeg onder 0,6 s blijft en er marge is voor de lage tonen. Hoe je zelf rekent, staat in [Hoeveel akoestische panelen heb je nodig?](/nl/blog/hoeveel-akoestische-panelen-nodig).

## Oplossingen die in een school standhouden

Een school is geen kantoor: er wordt tegen de wand geleund, met stoelen geschoven en met natte sponzen gewerkt. Onze aanbevelingen per zone:

- **Plafond eerst.** Een [akoestisch spanplafond](https://stretchplafond.be/products/acoustic-stretch-system) (naadloos, in één dag geplaatst door ons zusterbedrijf STRETCH) of plafondpanelen in gerecycleerd PET halen het grootste deel van de nagalm weg zonder wandoppervlak in te nemen.
- **Achterwand en zijwand.** [rPET Groove](/nl/products/rpet-groove): tien voorraadkleuren en elke kleur op bestelling, 12 / 24 / 36 mm (NRC 0,55 tot 0,90), brandklasse B-s1,d0 in wit, grijs en zwart en B-s2,d0 in de andere kleuren, te bewerken als MDF en makkelijk te vervangen per paneel. Op oorhoogte van de leerlingen, niet boven de kasten.
- **Kleuterklassen en zorglokalen.** [Interior](/nl/products/interior): modulaire textielpanelen (αw 1,0, klasse A) met een afneembare, wasbare hoes.
- **Refter, aula en polyvalente zaal.** [rWood Micro](/nl/products/rwood-micro): micro-geperforeerd houtfineer met brandklasse B-s1,d0, representatief genoeg voor de aula en absorberend genoeg voor de refter.
- **Open leeromgevingen.** [Divide](/nl/products/divide): vrijstaande, magnetisch koppelbare scheidingswanden die leerzones afbakenen zonder muren te bouwen.

## Brandklasse, emissies en onderhoud

Voor schoolgebouwen zijn de brandklasse en de binnenluchtkwaliteit even belangrijk als de akoestiek:

- **Brandklasse (EN 13501-1)**: de rPET-reeks (Panel, Groove, Flex Groove) is B-s1,d0 in wit, grijs en zwart en B-s2,d0 in de andere kleuren — beperkt brandbaar, weinig rook, geen brandende druppels; de rWood-reeks (Groove, Micro, Perf) is B-s1,d0 op de brandvertragende MDF-kern.
- **Emissies**: rPET is OEKO-TEX® Standard 100 gecertificeerd.
- **Onderhoud**: [PET-vilt](/nl/products/pet-akoestische-panelen) stofzuig je en behandel je plaatselijk bij vlekken; de hoezen van Interior gaan in de wasmachine. Beschadigde panelen vervang je per stuk, zonder de wand te herschilderen.
- **Herkomst**: het fineer van elk rWood-product is FSC®-gecertificeerd, chain-of-custody-certificaat C191539.

## Planning: zomervakantie, bestek en terugname

De meeste scholen plaatsen akoestiek in de zomervakantie. Reken terug vanaf de eerste schooldag:

1. **Meting en voorstel** in het voorjaar; goedkeuring door het schoolbestuur.
2. **Bestek**: voor elk product leveren we een bestektekst met αw, klasse, brandklasse, formaat, dikte en materiaal, zodat een aanbesteding zonder merknaam kan.
3. **Bestelling vóór eind mei**: rPET leveren we in ongeveer 3 à 4 werkweken (maatkleuren op aanvraag); rWood Micro en Perf in voorraadfineren in 3 à 4 weken, niet-voorraadfineren in 6 à 8 weken; de levertermijn van rWood Groove bevestigen we bij de bestelling.
4. **Plaatsing** in juli, meting in augustus, klaar voor september.

Aan het einde van de levensduur — of bij een verbouwing — nemen we de panelen gratis terug in België, Nederland, Frankrijk, Duitsland en Luxemburg; het materiaal gaat opnieuw onze productie in. Re-Sound is een merk van Stretch Group en produceert in eigen fabrieken in Beveren-Waas (België) en Częstochowa (Polen).

**Aan de slag?** Vraag het gratis stalenpakket aan (A4-stalen van elke reeks, verstuurd binnen drie werkdagen) en de bestekteksten voor je dossier, of boek een bezoek aan de [showroom](/nl/where-to-buy) in Beveren-Waas.

## Veelgestelde vragen

**Welke nagalmtijd is verplicht in een klaslokaal?**
NBN S 01-400-2 legt per lokaaltype en volume een maximale nominale nagalmtijd op; voor een standaardklas komt dat in de praktijk neer op ongeveer 0,6 tot 0,8 s (normaal comfort), strenger voor verhoogd comfort. Laat de exacte eis berekenen door je architect of akoesticus.

**Volstaan wandpanelen alleen in een klas?**
Meestal niet zonder veel wandoppervlak in te nemen: het plafond is het grootste vlak en het efficiëntst. Plafond plus achterwand is de combinatie die in de meeste klassen de norm haalt.

**Hoe snel kan een school geplaatst worden?**
rPET leveren we in ongeveer 3 à 4 werkweken; een gemiddelde klas plaatst een team op een halve dag. Bestel vóór eind mei om zeker in de zomervakantie te plaatsen.

*Bronnen: NBN S 01-400-2:2012 (akoestische criteria voor schoolgebouwen); DIN 18041:2016 (Hörsamkeit in Räumen); ISO 3382-2 (meting van de nagalmtijd); ISO 11654 (absorptieklassen); EN 13501-1 (brandclassificatie); productgegevens Re-Sound.*
