#!/usr/bin/env node
// ============================================================================
// LAYER 5 — REDIRECT VERIFICATION (the launch gate for the legacy Wix URLs).
//
//   npm run verify:redirects -- --base http://localhost:3999
//   npm run verify:redirects -- --base https://<preview>.vercel.app
//   npm run verify:redirects                       (production: URLs as listed)
//
// Reads legacy/wix-urls.txt, requests every URL with redirects followed (max
// 5 hops) and asserts that every chain ends in HTTP 200 with ZERO 4xx/5xx
// anywhere in the chain. Also tests, for a sample of URLs, the http→https,
// www→apex, trailing-slash and query-string variants, plus a set of invented
// junk paths that must land on the closest live index or the localised home
// (Layer 4, src/middleware.ts). Prints a per-URL report and exits non-zero
// on any failure. Mirrors scripts/verify-redirects.mjs on stretchgroup.be.
//
// Options
//   --base <origin>     Rewrite every inventory URL's origin to this base
//                       (a Vercel preview, or http://localhost:3999). Without
//                       it the URLs are requested exactly as listed
//                       (production, after the DNS cutover).
//   --file <path>       Inventory file (default legacy/wix-urls.txt). Text
//                       after "#" on a line is provenance and is ignored.
//   --host-header       With --base: also send the ORIGINAL host as the Host
//                       header, so the www → apex host rule in redirects.mjs
//                       can be exercised against a local server. Never use
//                       this against a Vercel preview (its router needs the
//                       preview host).
...
//                       host. Also read from VERCEL_AUTOMATION_BYPASS_SECRET.
//
// Every on-site chain must additionally end on https, on the apex host (no
// www.) and without a trailing slash — the canonical form the <link
// rel="canonical"> tags use.
// ============================================================================
...
const FILE = opt('--file', 'legacy/wix-urls.txt');
...
const MAX_HOPS = 5;
const APEX = 're-sound.be';

// Domains a chain may legitimately end on (or, in --external stop mode, hop
// to). Keep in sync with the group footer links and redirects.mjs targets.
const ALLOWED_HOSTS = new Set(['re-sound.be', 'stretchgroup.be', 'stretchplafond.be']);

// Every locale is routed (localePrefix 'always'): a junk path under a locale
// prefix must land on THAT locale's home; an unprefixed one goes through "/"
// where next-intl picks the language — with no Accept-Language header that is
// the default locale, en.
const LIVE_LOCALES = ['en', 'nl', 'fr', 'de', 'es', 'pt', 'da', 'sv', 'no', 'is'];
const DEFAULT_LOCALE = 'en';
function localeOf(pathname) {
  const seg = pathname.split('/').filter(Boolean)[0] ?? '';
  return LIVE_LOCALES.includes(seg) ? seg : null;
}
...
async function fetchOnce(url, headers) {
  ...
  try {
    ...
    const res = await fetch(url, {
      method: 'GET',
      redirect: 'manual',
      headers: { 'user-agent': 're-sound-verify-redirects/1.0', ...bypass, ...headers },
 ...
 * https host. `expect.path` pins the final pathname, `expect.prefix` its
 * start.
 */
async function follow(startUrl, headers, expect = {}) {
  ...
  for (let hop = 0; hop <= MAX_HOPS; hop++) {
    ...
    if (r.status >= 300 && r.status < 400) {
      ...
      if (leavesBase) {
        // Host-header runs: the www → apex 308 lands on re-sound.be — map it
        // back onto the base so the rest of the chain is exercised locally.
        if (BASE && HOST_HEADER && nextHost === APEX) {
    ...
    if (r.status === 200) {
      ...
      if (onSite) {
        ...
        // Every page lives under a locale prefix (localePrefix 'always').
        if (!localeOf(finalUrl.pathname)) return { ok: false, chain, reason: `final URL ${finalUrl.pathname} has no locale prefix` };
      }
      const finalPath = finalUrl.pathname.replace(/\/+$/, '') || '/';
      if (expect.path && finalPath !== expect.path) {
        return { ok: false, chain, reason: `expected ${expect.path}, landed on ${finalPath}` };
      }
      if (expect.prefix && !finalPath.startsWith(expect.prefix)) {
        return { ok: false, chain, reason: `expected a path under ${expect.prefix}, landed on ${finalPath}` };
      }
...
if (!flag('--no-variants')) {
  // A representative sample: one URL per family (locale prefix + first
  // section, so /en/product-page and /nl/product-page both count).
  const seen = new Set();
  const sample = [];
  for (const url of inventory) {
    const u = new URL(url);
    if (apexOf(u.host) !== APEX) continue;
    const segs = u.pathname.split('/').filter(Boolean);
    const family = (LIVE_LOCALES.includes(segs[0]) ? segs.slice(0, 2) : segs.slice(0, 1)).join('/') || '(root)';
  ...
  for (const url of sample) {
    ...
    // query string (Wix links carried utm_* and fbclid/gclid parameters)
    const q = new URL(url);
...
if (!flag('--no-junk')) {
  // [path, expected final path | { prefix }] — what Layer 4 must do with a
  // path nothing mapped. Unprefixed page-like paths go through "/" and end on
  // the default locale's home; section-like paths end on that section's
  // index; asset-like misses end on the (localised) home.
  const home = (p) => `/${localeOf(p) ?? DEFAULT_LOCALE}`;
  const junk = [
    ['/this-page-does-not-exist', home],
    ['/foo/bar/baz', home],
    ['/nl/deze-pagina-bestaat-niet', home],
    ['/fr/page-inexistante', home],
    ['/de/seite-fehlt', home],
    ['/is/sida-finnst-ekki', home],
    ['/wp-admin', home],
    ['/wp-login.php', home],
    ['/random-page.html', home],
    ['/old-brochure.pdf', home],
    ['/some/image.jpg', home],
    ['/_files/ugd/0123456789abcdef.pdf', home], // Wix media path
    ['/nl/oude-pagina.html', home],
    ['/nl/images/foto.jpg', home],
    ['/images/nope.jpg', home],
    ['/fonts/missing.woff2', home],
    ['/%E2%9C%93/unicode', home],
    ['/products/does-not-exist', '/en/products'],
    ['/en/products/does-not-exist', '/en/products'],
    ['/nl/products/bestaat-niet', '/nl/products'],
    ['/product-page/onbekend', '/en/products'],
    ['/nl/product-page/onbekend', '/nl/products'],
    ['/shop/whatever', '/en/products'],
    ['/post/onbekend-artikel', '/nl/blog'],
    ['/en/post/unknown-post', '/en/blog'],
    ['/nl/blog-1/onbekend', '/nl/blog'],
    ['/nl/blog/onbekend-artikel', '/nl/blog'],
    ['/en/blog/no-such-post', '/en/blog'],
    ['/en/guides/unknown-guide', { prefix: '/en/guides/' }],
    ['/nl/guides/onbekende-gids', { prefix: '/nl/guides/' }],
    ['/copy-of-home', '/nl'],
    ['/en/copy-of-solid', '/en'],
    ['/api/does-not-exist', home],
    ['/api/soap/?wsdl', home],
    ['/index.php/customer/account/', home],
  ];
  for (const [p, exp] of junk) {
    const u = (BASE || `https://${APEX}`) + p;
    const expect = typeof exp === 'function' ? { path: exp(p) } : typeof exp === 'string' ? { path: exp } : exp;
    tests.push({ group: 'junk→index', label: u, url: u, headers: {}, expect });
  }
}
...
const results = await runPool(tests, async (t) => {
  const r = await follow(t.url, t.headers, t.expect ?? {});
  return { ...t, ...r };
});
...
if (failures > 0) {
  console.error(`\n${failures} of ${results.length} checks FAILED — the legacy URL gate is red.`);
  process.exit(1);
}
