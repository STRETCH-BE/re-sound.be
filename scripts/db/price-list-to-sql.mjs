 ...
 * catalogue (supabase/migrations/0001_catalogue_and_orders.sql, prices as
 * decimal euros in `price_eur` since 0004_currencies.sql — the seed applies
 * after that migration; `price_cents` is generated from price_eur there and
 * is never inserted. The other currency columns are Michael's to fill in and
 * are left untouched by a re-import).
 *
...
/** Integer cents → the decimal euros price_eur holds: 411875 → 4118.75, -85625 → -856.25, null → null. */
const dec = (cents) => (cents == null ? 'null' : (cents / 100).toFixed(2));
...
lines.push('-- Prices go to price_eur (decimal euros, migration 0004); price_cents is generated from it and is not inserted.');
lines.push('begin;');
lines.push('');
...
lines.push('');
// price_eur only: the ISK / PLN / CHF / USD columns are Michael's, entered in
// the dashboard, and a re-import must not blank them.
const articleRow = (a) => `  (${q(a.code)}, ${q(a.productId)}, ${q(a.categoryKey)}, ${q(a.sheetCategory)}, ${q(a.group)}, ${q(a.description)}, ${j(a.labels ?? labelsFor(a.description))}, ${dec(a.priceCents)}, ${q(a.priceType)}, ${b(a.perSegment)}, ${b(a.perExtension ?? false)}, ${n(a.segments)}, ${b(a.isDefault)}, ${q(a.source)}, ${q(a.notes)}, ${a.sort}, ${b(a.active ?? true)})`;
const articlesInsert = (list) => `insert into public.articles (code, product_id, category_key, sheet_category, "group", description, labels, price_eur, price_type, per_segment, per_extension, segments, is_default, source, notes, sort, active) values
...
on conflict (code) do update set product_id = excluded.product_id, category_key = excluded.category_key, sheet_category = excluded.sheet_category,
  "group" = excluded."group", description = excluded.description, labels = excluded.labels, price_eur = excluded.price_eur, price_type = excluded.price_type,
...
//
// Currencies: the workbook is EUR only. `currencies` is written as migration
// 0004 seeds public.currencies (EUR active; ISK for 'is', PLN for 'pl', CHF,
// USD inactive) and every article's `prices` carries the EUR cents with the
// other four null — the database holds Michael's figures for those, and
// catalogue-snapshot.mjs is what brings them into the file.
// ---------------------------------------------------------------------------
const CURRENCIES = [
  { code: 'EUR', name: 'Euro', minorUnit: 2, symbol: '€', locales: [], active: true, sort: 1 },
  { code: 'ISK', name: 'Icelandic króna', minorUnit: 0, symbol: 'kr.', locales: ['is'], active: false, sort: 2 },
  { code: 'PLN', name: 'Polish złoty', minorUnit: 2, symbol: 'zł', locales: ['pl'], active: false, sort: 3 },
  { code: 'CHF', name: 'Swiss franc', minorUnit: 2, symbol: 'CHF', locales: [], active: false, sort: 4 },
  { code: 'USD', name: 'US dollar', minorUnit: 2, symbol: '$', locales: [], active: false, sort: 5 },
];
const pricesOf = (priceCents) => ({ EUR: priceCents, ISK: null, PLN: null, CHF: null, USD: null });
...
const snapshot = {
  ...
  articles: [...articles, ...serviceArticles].map((a) => ({
    ...
    description: a.description, labels: a.labels ?? labelsFor(a.description), priceCents: a.priceCents, prices: pricesOf(a.priceCents), priceType: a.priceType,
